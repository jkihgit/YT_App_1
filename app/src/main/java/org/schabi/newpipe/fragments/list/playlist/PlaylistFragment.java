...
public class PlaylistFragment extends BaseListInfoFragment<StreamInfoItem, PlaylistInfo> {
    ...
    public static PlaylistFragment getInstance(final int serviceId, final String url,
                                               final String name, final String ingress) {
        final PlaylistFragment instance = new PlaylistFragment(ingress);
        instance.setInitialData(serviceId, url, name);
        return instance;
    }

    public PlaylistFragment(final String ingress) {
        super(UserAction.REQUESTED_PLAYLIST);
        this.type = BaseListFragment.PLAYLIST_FRAGMENT_TYPE;
                ...
                headerBinding.uploaderLayout.setOnClickListener(v -> {
                    try {
                        NavigationHelper.openChannelFragment(getFM(), result.getServiceId(),
                                result.getUploaderUrl(), result.getUploaderName(), this.type);
